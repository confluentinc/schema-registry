...
public class KafkaAvroSerializer extends AbstractKafkaAvroSerializer
  ...
  @Override
  public byte[] serializeWithSchema(String topic, Headers headers, Object record,
      ParsedSchema schema) {
    if (record == null) {
      return null;
    }
    return serializeImpl(
        getSubjectName(topic, isKey, record, schema), topic, headers, record, (AvroSchema) schema);
  }

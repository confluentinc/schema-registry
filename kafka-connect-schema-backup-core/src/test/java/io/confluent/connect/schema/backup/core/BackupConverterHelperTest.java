...
public class BackupConverterHelperTest {
  ...
  // --- isBackupEnabled tests ---

  @Test
  public void isBackupEnabled_trueString_returnsTrue() {
  ...
  @Test
  public void isBackupEnabled_falseString_returnsFalse() {
    Map<String, String> config = new HashMap<>();
  ...
  @Test
  public void isBackupEnabled_uppercaseTrue_returnsTrue() {
    Map<String, String> config = new HashMap<>();
    config.put(SchemaBackupConfig.SCHEMA_BACKUP_ENABLED_CONFIG, "TRUE");
    assertTrue(BackupConverterHelper.isBackupEnabled(config));
  }
  ...
  @Test
  public void isBackupEnabled_mixedCaseTrue_returnsTrue() {
    Map<String, String> config = new HashMap<>();
    config.put(SchemaBackupConfig.SCHEMA_BACKUP_ENABLED_CONFIG, "True");
    assertTrue(BackupConverterHelper.isBackupEnabled(config));
  }
  ...
  @Test
  public void isBackupEnabled_missingKey_returnsFalse() {
    assertFalse(BackupConverterHelper.isBackupEnabled(Collections.emptyMap()));
  }

  @Test
  public void isBackupEnabled_nullValue_returnsFalse() {
    Map<String, String> config = new HashMap<>();
  ...
  @Test
  public void isBackupEnabled_emptyString_returnsFalse() {
    Map<String, String> config = new HashMap<>();
  ...
  @Test
  public void isBackupEnabled_nonBooleanString_returnsFalse() {
    Map<String, String> config = new HashMap<>();
    config.put(SchemaBackupConfig.SCHEMA_BACKUP_ENABLED_CONFIG, "yes");
    assertFalse(BackupConverterHelper.isBackupEnabled(config));
  }

  // --- getReferenceResolver tests ---

  @Test
  public void getReferenceResolver_returnsNonNull() {
    assertNotNull(helper.getReferenceResolver());
  }

  @Test
  public void getReferenceResolver_returnsSameInstance() {
    assertSame(helper.getReferenceResolver(), helper.getReferenceResolver());
  }

  // --- wrapWithBackupMetadata tests ---

  @Test
  public void wrapWithBackupMetadata_simpleSchema_wrapsCorrectly()
      throws Exception {
    AvroSchema avro = new AvroSchema(USER_SCHEMA);
    int schemaId = schemaRegistry.register(TOPIC + "-value", avro);
  ...
  @Test
  public void wrapWithBackupMetadata_withReferences_includesRefTree()
      throws Exception {
    // Register the referenced schema first
    AvroSchema addrSchema = new AvroSchema(ADDRESS_SCHEMA);
    int addrId = schemaRegistry.register("com.example.Address", addrSchema);

    // Register the main schema with a reference
    SchemaReference ref = new SchemaReference(
        "com.example.Address", "com.example.Address", 1);
    AvroSchema orderSchema = new AvroSchema(
        ORDER_SCHEMA,
        Collections.singletonList(ref),
        Collections.singletonMap("com.example.Address", ADDRESS_SCHEMA),
        null);
    int orderId = schemaRegistry.register(TOPIC + "-value", orderSchema);
  ...
  @Test
  public void wrapWithBackupMetadata_noReferences_nullRefFields()
      throws Exception {
    AvroSchema avro = new AvroSchema(USER_SCHEMA);
    int schemaId = schemaRegistry.register(TOPIC + "-value", avro);
  ...
  @Test
  public void wrapWithBackupMetadata_nullOriginalSchema_buildsWrapper()
      throws Exception {
    AvroSchema avro = new AvroSchema(USER_SCHEMA);
    int schemaId = schemaRegistry.register(TOPIC + "-value", avro);
  ...
  @Test
  public void wrapWithBackupMetadata_cachedSchema_reusesEntry()
      throws Exception {
    AvroSchema avro = new AvroSchema(USER_SCHEMA);
    int schemaId = schemaRegistry.register(TOPIC + "-value", avro);

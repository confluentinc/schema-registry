...
public class BoundedConcurrentHashMap<K, V> extends AbstractMap<K, V>
  ...
  static final class LRU<K, V> extends LinkedHashMap<HashEntry<K, V>, V> implements
      EvictionPolicy<K, V> {
    ...
    public LRU(Segment<K, V> s, int capacity, float lf, int maxBatchSize,
  ...
  /**
   * Adapted to Infinispan BoundedConcurrentHashMap using LIRS implementation ideas from Charles Fry
   * (fry@google.com) See http://code.google.com/p/concurrentlinkedhashmap/source/browse/trunk/src/test/java/com/googlecode/concurrentlinkedhashmap/caches/LirsMap.java
  ...
  private static final class LIRSHashEntry<K, V> extends HashEntry<K, V> {
     ...
     * both when a new entry is first created, and when a non-resident entry is re-computed.
     */
    ...
    /**
     * Temporarily removes this entry from the stack, fixing up neighbor links. This entry's links
     * remain unchanged, meaning that {@link #inStack()} will continue to return true. This should
     * only be called if this node's links will be subsequently changed.
     */
    ...
    /**
     * Temporarily removes this entry from the queue, fixing up neighbor links. This entry's links
     * remain unchanged. This should only be called if this node's links will be subsequently
     * changed.
     */
    ...
    /**
     * Moves this entry from the stack to the queue, marking it cold (as hot entries must remain in
     * the stack). This should only be called on resident entries, as non-resident entries should
     * not be made resident. The bottom entry on the queue is always hot due to stack pruning.
     */
    ...
    /**
     * Moves this entry from the queue to the stack, marking it hot (as cold resident entries must
     * remain in the queue).
     */
    ...
    /**
     * Evicts this entry, removing it from the queue and setting its status to cold non-resident. If
     * the entry is already absent from the stack, it is removed from the backing map; otherwise it
     * remains in order for its recency to be maintained.
     */
    private void evict() {
      removeFromQueue();
      removeFromStack();
    ...
    /**
     * Removes this entry from the cache. This operation is not specified in the paper, which does
     * not account for forced eviction.
     */
    private V remove() {
  ...
  static final class LIRS<K, V> implements EvictionPolicy<K, V> {
    ...
    /**
     * The accessQueue for reducing lock contention See "BP-Wrapper: a system framework making any
    ...
    /**
     * Prunes HIR blocks in the bottom of the stack until an HOT block sits in the stack bottom. If
     * pruned blocks were resident, then they remain in the queue; otherwise they are no longer
     * referenced, and are thus removed from the backing map.
     */
  ...
  public BoundedConcurrentHashMap(
    ...
    concurrencyLevel = Math
        .min(capacity / 2, concurrencyLevel); // concurrencyLevel cannot be > capacity/2
    ...
    for (int i = 0; i < this.segments.length; ++i) {
      this.segments[i] = new Segment<K, V>(cap, c, DEFAULT_LOAD_FACTOR, evictionStrategy,
          evictionListener);
    }
  }

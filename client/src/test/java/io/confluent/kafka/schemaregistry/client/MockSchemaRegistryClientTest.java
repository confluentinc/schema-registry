...
public class MockSchemaRegistryClientTest {
    ...
    public void testUpdateOneAssociationViaUpsertEndpoint() {
      ...
      createRequest = new AssociationRequestBuilder().defaultResource().valueSubject(defaultValueSubject)
              .valueLifecycle(LifecyclePolicy.WEAK).valueSchema(EVOLVED_AVRO_SCHEMA).build();
      try {
        client.createOrUpdateAssociation(createRequest);
      } catch (Exception e) {
      ...
      createRequest = new AssociationRequestBuilder().defaultResource().valueSubject(defaultValueSubject)
              .valueLifecycle(LifecyclePolicy.WEAK).valueFrozen(true).build();
      try {
        client.createOrUpdateAssociation(createRequest);
      ...
      createRequest = new AssociationRequestBuilder().defaultResource().valueSubject(defaultValueSubject)
              .valueLifecycle(LifecyclePolicy.STRONG).valueFrozen(false).build();
      try {
        client.createOrUpdateAssociation(createRequest);
      } catch (Exception e) {
      ...
      // Create key weak association
      createRequest = new AssociationRequestBuilder().defaultResource().keySubject(defaulKeySubject)
              .keyLifecycle(LifecyclePolicy.WEAK).keySchema(SIMPLE_AVRO_SCHEMA).build();
      try {
        client.createOrUpdateAssociation(createRequest);
      } catch (Exception e) {
        assertNull("AssociationCreateOrUpdateRequest should succeed.", e);
      ...
      // Change value association lifecycle weak to strong should fail.
      createRequest = new AssociationRequestBuilder().defaultResource().keySubject(defaultValueSubject)
              .valueLifecycle(LifecyclePolicy.STRONG).build();
      try {
        client.createOrUpdateAssociation(createRequest);
        fail("This weak to strong operation should fail.");
      } catch (Exception e) {
        assertNotNull("Change weak to strong should fail because the subject has a second weak association", e);
      }
    }

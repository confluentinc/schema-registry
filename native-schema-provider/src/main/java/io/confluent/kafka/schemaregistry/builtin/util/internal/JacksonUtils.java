...
public class JacksonUtils {
  ...
  /**
   * This object mapper uses a special variant that has different visibility rules, used in
   * objectToMap method.
   */
  ...
  static void toJson(Object datum, JsonGenerator generator) throws IOException {
    if (datum == JsonProperties.NULL_VALUE) { // null
      ...
      generator.writeBinary(
          (byte[]) datum);// writeString(new String((byte[]) datum, StandardCharsets.ISO_8859_1));
  ...
  public static Object toObject(JsonNode jsonNode, Schema schema) {
    ...
    if (jsonNode == null) {
      ...
      if (schema == null || schema.getType().equals(Schema.Type.INT32)) {
        return jsonNode.asInt();
      } else if (schema.getType().equals(Schema.Type.INT64)) {
        return jsonNode.asLong();
      } else if (schema.getType().equals(Schema.Type.FLOAT32)) {
        return (float) jsonNode.asDouble();
      } else if (schema.getType().equals(Schema.Type.FLOAT64)) {
        return jsonNode.asDouble();
      }
    } else if (jsonNode.isLong()) {
      if (schema == null || schema.getType().equals(Schema.Type.INT64)) {
        return jsonNode.asLong();
      } else if (schema.getType().equals(Schema.Type.INT32)) {
      ...
      if (schema != null) {
        if (schema.getType().equals(Schema.Type.FLOAT64)) {
      ...
      if (schema == null
          || schema.getType().equals(Schema.Type.CHAR)
          || schema.getType().equals(Type.STRING)
          || schema.getType().equals(Schema.Type.ENUM)) {
        return jsonNode.asText();
      } else if (schema.getType().equals(Schema.Type.BINARY)
      ...
      for (Iterator<String> it = jsonNode.fieldNames(); it.hasNext(); ) {
        ...
        } else if (schema != null && schema.getType().equals(Schema.Type.STRUCT)) {

...
/**
 * Thrown to prevent making large allocations when reading potentially pathological input data from
 * an untrusted source.
 * <p/>
 * The following system properties can be set to limit the size of bytes, strings and collection
...
public class SystemLimitException extends SchemaRuntimeException {
  ...
  /**
   * The maximum length of array to allocate (unless necessary). Some VMs reserve some header words
  ...
  /**
   * Get an integer value stored in a system property, used to configure the system behaviour of
   * decoders
   *
   * @param property     The system property to fetch
   * @param defaultValue The value to use if the system property is not present or parsable as an
   *                     int
  ...
  private static int getLimitFromProperty(String property, int defaultValue) {
    ...
    if (o != null) {
      try {
        ...
        LoggerFactory.getLogger(SystemLimitException.class)
            .warn("Could not parse property " + property + ": " + o,
                nfe);
      }
    }
    return i;
   ...
   * @return The size of the bytes if and only if it is within the limit and non-negative.
   * @throws UnsupportedOperationException if reading the datum would allocate a collection that the
   *                                       Java VM would be unable to handle
   * @throws SystemLimitException          if the decoding should fail because it would otherwise
   *                                       result in an allocation exceeding the set limit
   * @throws SchemaRuntimeException        if the length is negative
   */
  public static int checkMaxBytesLength(long length) {
    if (length < 0) {
      throw new SchemaRuntimeException("Malformed data. Length is negative: " + length);
    }
  ...
  /**
   * Check to ensure that reading the specified number of items remains within the specified
   * limits.
   *
   * @param existing The number of elements items read in the collection
   * @param items    The next number of items to read. In normal usage, this is always a positive,
  ...
  public static int checkMaxCollectionLength(long existing, long items) {
    ...
    if (existing < 0) {
      throw new SchemaRuntimeException("Malformed data. Length is negative: " + existing);
    }
    if (items < 0) {
      throw new SchemaRuntimeException("Malformed data. Length is negative: " + items);
    }
   ...
   * @return The size of the string if and only if it is within the limit and non-negative.
   * @throws UnsupportedOperationException if reading the items would allocate a collection that the
   *                                       Java VM would be unable to handle
   * @throws SystemLimitException          if the decoding should fail because it would otherwise
   *                                       result in an allocation exceeding the set limit
  ...
  public static int checkMaxStringLength(long length) {
    ...
    if (length > MAX_ARRAY_VM_LIMIT) {
      throw new UnsupportedOperationException(
          "Cannot read strings longer than " + MAX_ARRAY_VM_LIMIT + " bytes");
    }

...
/**
 * Conversion helpers backing {@code to_decimal} and the {@code decimal_*} operator
 * functions. The CEL surface treats Decimal as the canonical type
 * {@link CelTypes#DECIMAL}; this client backs it with {@link BigDecimal}.
 */
final class DecimalUtils {
  ...
  /**
   * Runtime dispatch backing {@code to_decimal(dyn)}. Accepts whatever shape a Proto/Avro
   * decoder commonly produces. Throws {@link IllegalArgumentException} with a hint when
   * the input shape lacks the schema-side metadata needed (raw bytes need a scale; pass it
   * via the two-arg overload).
   */
    ...
    if (o instanceof byte[] || o instanceof ByteBuffer || o instanceof ByteString) {
      throw new IllegalArgumentException(
          "Cannot convert raw bytes to Decimal without a scale; use "
              + "to_decimal(bytes, scale) or set useLogicalTypeConverters=true on the "

...
public class KafkaJsonSchemaSerializer<T> extends AbstractKafkaJsonSchemaSerializer<T>
  ...
  @Override
  public byte[] serializeWithSchema(String topic, Headers headers, T record, ParsedSchema schema) {
    if (record == null) {
      return null;
    }
    Object value = JsonSchemaUtils.getValue(envelopeDetection, record);
    return serializeImpl(
        getSubjectName(
            topic, isKey, value, schema), topic, headers, (T) value, (JsonSchema) schema);
  }

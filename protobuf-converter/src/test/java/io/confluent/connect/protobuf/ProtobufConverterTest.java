...
public class ProtobufConverterTest {
  ...
  public void testComplexWrappers() {
    ...
    Struct original = new Struct(schema).put("int32_null", null)
        ...
        .put("int32_opt", 12)
        .put("int64_opt", 12L)
        .put("float32_opt", 12.2f)
        .put("float64_opt", 12.2)
        .put("boolean_opt", true)
        .put("string_opt", "foo")
        .put("bytes_opt", ByteBuffer.wrap("foo".getBytes()))
        .put("int32", 13)
        .put("int64", 13L)
        .put("float32", 13.3f)
        .put("float64", 13.3)
        .put("boolean", false)
        .put("string", "bar")
        .put("bytes", ByteBuffer.wrap("bar".getBytes()))
        .put("array", Arrays.asList("a", "b", "c"))
        .put("map", Collections.singletonMap("field", 1));
    // Because of registration in schema registry and lookup, we'll have added a version number
    Schema expectedSchema = builder.name("ConnectDefault1").version(1).build();
    Struct expected = new Struct(expectedSchema).put("int32_null", null)
        ...
        .put("boolean_opt", true)
        .put("string_opt", "foo")
        .put("bytes_opt", ByteBuffer.wrap("foo".getBytes()).asReadOnlyBuffer())
        .put("int32", 13)
        .put("int64", 13L)
        .put("float32", 13.3f)
        .put("float64", 13.3)
        .put("boolean", false)
        .put("string", "bar")
        .put("bytes", ByteBuffer.wrap("bar".getBytes()).asReadOnlyBuffer())
        .put("array", Arrays.asList("a", "b", "c"))
        .put("map", Collections.singletonMap("field", 1));

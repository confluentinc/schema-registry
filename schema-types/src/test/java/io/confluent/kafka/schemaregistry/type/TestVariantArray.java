...
public class TestVariantArray {
  ...
  private static byte[] constructArray(byte[]... elements) {
    ...
    output.put(VariantFormat.arrayHeader(isLarge, offsetSize));
    ...
    for (int i = 0; i < elements.length; ++i) {
      VariantTestUtils.writeVarlenInt(output, currOffset, offsetSize);
  ...
  public void testEmptyArray() {
    Variant value = new Variant(ByteBuffer.wrap(new byte[] {0b0011, 0x00}), VariantTestUtils.EMPTY_METADATA);
  ...
  public void testEmptyLargeArray() {
    Variant value = new Variant(
        ByteBuffer.wrap(new byte[] {0b10011, 0x00, 0x00, 0x00, 0x00}), VariantTestUtils.EMPTY_METADATA);
    VariantTestUtils.testVariant(value, v -> {
      VariantTestUtils.checkType(v, VariantFormat.ARRAY, Variant.Type.ARRAY);
      Assert.assertEquals(0, v.numArrayElements());
    });
  ...
  public void testMixedArray() {
    ...
    VariantTestUtils.testVariant(value, v -> {
      ...
      VariantTestUtils.checkType(v.getElementAtIndex(0), VariantFormat.PRIMITIVE, Variant.Type.DATE);
      Assert.assertEquals(
      ...
      VariantTestUtils.checkType(v.getElementAtIndex(3), VariantFormat.PRIMITIVE, Variant.Type.STRING);
      Assert.assertEquals("variant", v.getElementAtIndex(3).getString());
      VariantTestUtils.checkType(v.getElementAtIndex(4), VariantFormat.ARRAY, Variant.Type.ARRAY);

      Variant nestedV = v.getElementAtIndex(4);
      Assert.assertEquals(3, nestedV.numArrayElements());
      VariantTestUtils.checkType(nestedV.getElementAtIndex(0), VariantFormat.PRIMITIVE, Variant.Type.INT);
      Assert.assertEquals(1234567890, nestedV.getElementAtIndex(0).getInt());
      VariantTestUtils.checkType(nestedV.getElementAtIndex(1), VariantFormat.PRIMITIVE, Variant.Type.NULL);
      VariantTestUtils.checkType(nestedV.getElementAtIndex(2), VariantFormat.SHORT_STR, Variant.Type.STRING);
  ...
  private void testArrayOffsetSize(String randomString) {
    Variant value = new Variant(
        ByteBuffer.wrap(constructArray(VariantTestUtils.constructString(randomString), VALUE_BOOL, VALUE_INT)),
        VariantTestUtils.EMPTY_METADATA);
  ...
  public void testInvalidArray() {
    try {
      // An object header
      Variant value = new Variant(ByteBuffer.wrap(new byte[] {0b1000010}), VariantTestUtils.EMPTY_METADATA);

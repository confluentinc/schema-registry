...
public class DataProductInfo {
  ...
  public void updateHash(MessageDigest md) {
    ...
    if (owner != null) {
      md.update(owner.getBytes(StandardCharsets.UTF_8));
    }
    if (contact != null) {
      md.update(contact.getBytes(StandardCharsets.UTF_8));
    }
  }
  ...
  private static void validateName(String name) throws RuleException {
    if (name == null) {
      throw new RuleException("Missing data product name");
    }
    int length = name.length();
    if (length == 0) {
      throw new RuleException("Empty data product name");
    }
    if (length > NAME_MAX_LENGTH) {
      throw new RuleException("Data product name too long");
    }
    char first = name.charAt(0);
    if (!(Character.isLetter(first) || first == '_')) {
      throw new RuleException("Illegal initial character in data product name: " + name);
    }
    for (int i = 1; i < length; i++) {
      char c = name.charAt(i);
      if (!(Character.isLetterOrDigit(c) || c == '_' || c == '-')) {
        throw new RuleException("Illegal character in data product name: " + name);
      }
    }
  }
}

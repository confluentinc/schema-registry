...
/**
 * A repository for mocked Data Product Registry clients, to aid in testing.
 *
 * <p>Logically independent "instances" of mocked Data Product Registry are created or retrieved
 * via named scopes {@link MockDataProductRegistryClientFactory#getClientForScope(String, Map)}.
 ...
 * {@link MockDataProductRegistryClientFactory#dropScope(String)}.
 ...
 * {@code MockDataProductRegistryClientFactory.getClientForScope("my-scope-name", configs)}.
 */
public final class MockDataProductRegistryClientFactory {
  private static final String MOCK_URL_PREFIX = "mock://";
  private static final Map<String, DataProductRegistryClient> SCOPED_CLIENTS = new HashMap<>();

  // Not instantiable. All access is via static methods.
  private MockDataProductRegistryClientFactory() {
   ...
   */
  public static DataProductRegistryClient getClientForScope(
      final String scope, Map<String, ?> configs) {
    synchronized (SCOPED_CLIENTS) {
      if (!SCOPED_CLIENTS.containsKey(scope)) {
        SCOPED_CLIENTS.put(scope, new MockDataProductRegistryClient(configs));
      }
    }
